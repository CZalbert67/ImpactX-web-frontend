...
import type { AlertItem } from "@/features/platform/types";
...
export function AlertsPage() {
  ...
  return <div className="space-y-6">
    ...
    {query.data && query.data.length > 0 ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{query.data.map((item) => <Card key={item.id}>
      ...
      <div className="mt-4 flex items-center justify-between"><Badge tone={tone(item.estado)}>{item.estado}</Badge><Button size="sm" variant="outline" onClick={() => setSelected(item)}>Ver detalle</Button></div>
    </Card>)}</div> : null}
    <Modal open={Boolean(selected)} onClose={() => setSelected(null)} title="Detalle de alerta" description="Información registrada por el backend de ImpactX.">
